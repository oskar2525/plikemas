...
class LoginController extends Controller
{
    ...
    public function loginPost(Request $request)
    {
        ...
        if ($data) {
            if (Hash::check($password,$data->password)) {
                ...
                return redirect('login')->with('error','username atau password tidak ditemukan');
            }
        }else{
            return redirect('login')->with('error','username atau password tidak ditemukan');
        }
    ...
    public function registerMasyarakatPost(Request $request){
        $this->validate($request, [
        	'nik' => 'required|min:16|unique:masyarakat',
            'nama' => 'required|min:4',
        ...
        if ($saveID) {
        	...
        	if ($id) {
        		...
        		if (Hash::check($password,$id->password)) {
			        ...
			        if ($status) {
			        	return redirect('admin/home')->with('success','Kamu berhasil Register silahkan isi Login');
			        } else{
